...
IRSend.prototype.send_multiple = function(count,remote, code, callback) {
  var command = this._send_multiple(remote, code);
  return exec(command, callback);
};
...
IRSend.prototype._send_multiple = function(count, remote, code) {
  ...
  return this.command + '--count=' + count +' SEND_ONCE "' + remote + '" "' + code + '"';
};
